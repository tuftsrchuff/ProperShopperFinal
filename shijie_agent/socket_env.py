...
# This is a modified version of the original socket_env
# The main difference is that I added a new action called "HACK"
# It's basically a reset but gives a cart to the agent directly after resetting
    ...
    def handle_exploratory_events(self):
        ...
        if self.keyboard_input:
            ...
            if keys[pygame.K_UP]:  # up
                ...
                obs, reward, done, info, violations=self.env.step(self.single_player_action(PlayerAction.EAST))
                # print(obs['players'][0]['position'])
...
if __name__ == "__main__":
    ...
    # new_cart = Cart(0,0,player,Direction.SOUTH)
    # new_cart.update_position(player.position[0], player.position[1])
    # env.unwrapped.game.carts.append(new_cart)
    # env.unwrapped.game.objects.append(new_cart)
                        ...
                        if len(recv_data) < 4096:
                            ...
                            if is_single_player(command):
                                ...
                                if command in ACTION_COMMANDS:
                                    ...
                                    action_id = ACTION_COMMANDS.index("RESET")
                                    curr_action[player] = (action_id,0)
                                    while valid_state_flag == False:
                                        valid_state_flag = True
                                        obs, reward, done, info, violations = env.step(tuple(curr_action))
                                        player = env.unwrapped.game.players[0]
                                        new_cart = Cart(0,0,player,Direction.EAST)
                                        new_cart.update_position(player.position[0], player.position[1])
                                        env.unwrapped.game.carts.append(new_cart)
                                        ...
                                        for obj in env.unwrapped.game.objects:
                                            if obj.collision(new_cart, new_cart.position[0], new_cart.position[1]):
                                                valid_state_flag = False
                                                print("cart hit object when resetting!, {},{}".format(new_cart.position[0],new_cart.position[1]))
                                            elif env.unwrapped.game.hits_wall(new_cart, new_cart.position[0], new_cart.position[1]):
                                                valid_state_flag = False
                                                print("cart hit wall when resetting!")
                                        if valid_state_flag == True:
                                            env.unwrapped.game.objects.append(new_cart)
                                            break

                                    json_to_send = get_action_json("RESET", env, obs, reward, done, info, violations)
